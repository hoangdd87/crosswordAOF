...
class PDOHelper {
	...
	public function insert_User_Message( $user_message ) {
		...
		$sth = $this->PDO->prepare( "INSERT INTO user_message(user_name, message, time,isdelivery) 
              									VALUES (:user_name, :message, :time,:isdelivery)" );
		...
		$sth->bindParam( ':isdelivery', $user_message->isdelivery );
	...
	public function get_All_Questions() {
		...
		$sth = $this->PDO->prepare( "SELECT question_id, question, correct_answer, status, 
									answer_time, time_begin, time_end, answer_a, answer_b, answer_c, answer_d
	...
	public function get_User_Message_Not_Seen() {
		include_once __DIR__ . '/../model/User_Message.php';
		$sth = $this->PDO->prepare( "SELECT  user_message.user_name, message, time, isdelivery,teamname FROM user_message INNER JOIN users
ON user_message.user_name=users.user_name
                          WHERE isdelivery=0" );
		$sth->execute();
	...
	public function clear_user_message() {
		$sth = $this->PDO->prepare( "DELETE FROM user_message WHERE 1" );
	 ...
	 * @return boolean
	 */
	public function update_table( $table_name, $params ) {
